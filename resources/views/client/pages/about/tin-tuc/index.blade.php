...
<!-- PAGE HERO -->
  <section class="relative min-h-[320px] sm:min-h-[380px] flex items-center overflow-hidden" style="background: linear-gradient(135deg, #10203C 0%, #193877 60%, #204DA4 100%);">
    <div class="absolute inset-0 z-0">
      <img src="/assets/images/hero-office.jpg" alt="Tin tức – Dự án và sự kiện" class="w-full h-full object-cover object-center opacity-25" loading="eager" />
    ...
    <div class="relative z-10 max-w-[1370px] mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-16 w-full text-center">
      <span class="font-handwriting text-2xl sm:text-3xl text-[#5eb74c] font-bold block mb-2">Tin tức</span>
      <h1 class="text-2xl sm:text-[38px] lg:text-[42px] font-bold text-white mb-4 leading-tight tracking-tight drop-shadow-sm">Tin tức – Dự án và sự kiện</h1>
      <p class="max-w-3xl mx-auto text-gray-200 text-[15px] sm:text-[16px] leading-relaxed">Cập nhật hoạt động triển khai, bàn giao và các sự kiện liên quan của Hương Sơn.</p>
            ...
            <article class="border border-gray-200/80 group flex flex-col rounded-sm overflow-hidden" style="background-color: rgb(247, 243, 238);">
              ...
              <div class="p-6 flex flex-col flex-1">
                <div class="flex items-center justify-between text-[11px] text-gray-500 mb-2">
                  <span class="font-bold uppercase tracking-[0.15em] text-[#1A9900]">Tin hoạt động</span>
                  <span>{{ $post->published_at ? $post->published_at->format('d/m/Y') : '' }}</span>
                </div>
                <h3 class="text-[17px] font-bold text-[#181923] mb-2.5 group-hover:text-[#1A9900] transition leading-snug line-clamp-2">
                  <a href="/ve-huong-son/tin-tuc/{{ $post->slug }}/">{{ $post->title }}</a>
                </h3>
                <p class="text-gray-500 text-[14px] leading-relaxed mb-4 flex-1 line-clamp-3">
                  {{ $post->summary ?: 'Xem chi tiết thông tin và hình ảnh triển khai thực tế từ Hương Sơn.' }}
                </p>
                <a href="/ve-huong-son/tin-tuc/{{ $post->slug }}/" class="inline-flex items-center space-x-1 text-[#1A9900] font-bold text-xs uppercase tracking-wider hover:underline">
