...
def head(*, title, description, url, keywords="", og_type="website", jsonld=None,
         robots="index,follow"):
    """<head> đầy đủ: canonical, OG, Twitter, JSON-LD."""
    canonical = SITE["website"].rstrip("/") + url
    blocks = ""
...
def header(active=""):
    ...
    for m in NAV:
        ...
        color = f"text-[{BRAND}]" if is_active else f"text-white hover:text-[{BRAND}]"
        if m.get("children"):
            links = "".join(
                f'<a href="{c["url"]}" class="block px-4 py-2.5 text-sm text-gray-300 '
                f'hover:bg-[{BRAND}] hover:text-white transition">{c["label"]}</a>'
                for c in m["children"])
            items += f"""
        <div class="relative has-dropdown group py-2">
          <a href="{m['url']}" class="nav-link {color} font-medium text-sm flex items-center space-x-1 group-hover:text-[{BRAND}] transition">
            <span>{m['label']}</span>
            <i class="fa-solid fa-chevron-down text-[10px] transition-transform duration-200 group-hover:rotate-180"></i>
          </a>
          <div class="dropdown-menu absolute top-full left-0 w-80 bg-[{DARK}] border border-gray-800 shadow-2xl py-2 z-50">{links}
          </div>
        </div>"""
        else:
            items += (f'\n        <a href="{m["url"]}" class="nav-link {color} '
                      f'font-medium text-sm transition py-2">{m["label"]}</a>')
    return f"""
  <!-- MAIN HEADER -->
  <header class="site-header bg-[{DARK}] w-full z-40 transition-all duration-300 border-b border-gray-800/50">
    <div class="max-w-[1370px] mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
      ...
      <div class="flex xl:hidden items-center space-x-3">
        <button class="search-toggle w-9 h-9 bg-gray-800 text-white flex items-center justify-center hover:bg-[{BRAND}]" aria-label="Tìm kiếm">
...
def footer():
    def col(title, links, span):
        li = "".join(f'<li><a href="{u}" class="hover:text-[{BRAND}] transition block">{l}</a></li>'
                     for l, u in links)
        return f"""
        <div class="lg:col-span-{span}">
          <h4 class="text-white text-[16px] font-bold mb-5 uppercase tracking-wider">{title}</h4>
          <ul class="space-y-3 text-[14.5px] font-normal text-gray-300">{li}</ul>
        </div>"""

    products = [(c["label"], c["url"]) for c in NAV[0]["children"][:6]]
    solutions = [(c["label"], c["url"]) for c in NAV[1]["children"][:6]]
    services = [(c["label"], c["url"]) for c in NAV[2]["children"]] + \
               [("Dự án – Case study", "/du-an/"), ("Nhận tư vấn", "/nhan-tu-van/")]
    phones = "".join(
        f'''<li class="flex items-center space-x-3">
              <i class="fa-solid fa-phone text-[{BRAND}] text-sm flex-shrink-0"></i>
              <a href="tel:{h["tel"]}" data-ga="click_hotline" class="hover:text-[{BRAND}] transition font-medium">{h["label"]}</a>
              <span class="text-gray-500 text-[13px]">{h["note"]}</span>
            </li>''' for h in SITE["hotlines"])

    return f"""
  <!-- FOOTER -->
  <footer class="bg-[{DARK}] text-gray-300 pt-20 pb-10 border-t border-gray-800 relative bg-cover bg-center" style="background-image: linear-gradient(rgba(24, 25, 36, 0.9), rgba(24, 25, 36, 0.97)), url('/assets/images/xxx_footer_xxx.jpg');">
    <div class="max-w-[1370px] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 pb-14 border-b border-gray-800/80">
        ...
        <div class="lg:col-span-4 space-y-5">
          <a href="/" class="inline-block">{_logo()}</a>
          <p class="text-[15px] text-gray-300 leading-relaxed max-w-md">
            {SITE['legal_name']} — {SITE['positioning']}.
          </p>
          <p class="text-[13.5px] text-gray-400">Mã số thuế: {SITE['mst']} · Thành lập {SITE['founded']}</p>
          <div class="flex flex-wrap gap-2 pt-1">
            {"".join(f'<span class="text-[11px] font-bold tracking-wider text-gray-300 border border-gray-700 px-2.5 py-1">{b}</span>' for b in SITE["brands"])}
          </div>
        </div>
...
def drawer():
    ...
    for m in NAV:
        if m.get("children"):
            subs = "".join(f'<a href="{c["url"]}" class="block py-1 hover:text-white">{c["label"]}</a>'
                           for c in m["children"])
            items += f"""
      <div>
        <button class="mobile-dropdown-btn w-full flex items-center justify-between text-white font-medium py-2 border-b border-gray-800/50 hover:text-[{BRAND}]">
          <span>{m['label']}</span>
          <i class="fa-solid fa-chevron-down text-xs"></i>
        </button>
        <div class="hidden pl-4 py-2 space-y-2 text-sm text-gray-400">
          <a href="{m['url']}" class="block py-1 text-[{BRAND}] font-medium">Tổng quan</a>{subs}
        </div>
      </div>"""
        else:
            items += (f'\n      <a href="{m["url"]}" class="block text-white font-medium py-2 '
                      f'border-b border-gray-800/50 hover:text-[{BRAND}]">{m["label"]}</a>')
    return f"""
  <!-- MOBILE DRAWER -->
  <div id="mobile-backdrop" class="fixed inset-0 bg-black/60 z-50 hidden opacity-0 transition-opacity duration-300"></div>
  <div id="mobile-drawer" class="fixed top-0 left-0 bottom-0 w-80 max-w-[85vw] bg-[{DARK}] z-50 shadow-2xl transform -translate-x-full transition-transform duration-300 flex flex-col">
  ...
  <div id="search-modal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300 flex items-center justify-center p-4">
    <div class="bg-[{DARK}] border border-gray-800 w-full max-w-2xl p-6 sm:p-8 shadow-2xl relative">
      ...
      <form action="/ve-huong-son/kien-thuc/" method="get" class="relative">
        <input type="text" id="search-input" name="s" placeholder="VD: thuê máy in đề thi, máy scan tốc độ cao..." class="w-full bg-gray-900 border border-gray-700 text-white px-4 py-3 pr-12 text-sm focus:outline-none focus:border-[{BRAND}]" />
  ...
  <a href="{SITE['zalo']}" target="_blank" rel="noopener" data-ga="click_zalo" class="fixed right-6 z-40 w-12 h-12 bg-[#0068ff] text-white flex items-center justify-center shadow-xl animate-pulse-zalo" style="bottom: 96px;" title="Chat Zalo">
